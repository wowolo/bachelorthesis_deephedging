# -*- coding: utf-8 -*-
"""
Created on Wed Sep  2 09:28:08 2020
...
from black_scholes_price import model_prices
from util_bs import custom_loss, input_constructor



# Define fixed paramters for framework of every experiment:
n=30 #time-steps
d=1 #number of trading instruments - for Chacko Viceira price: d=2 and for Black Scholes price: d=1
...
class neural_network():
    ...
    def __init__(self, eps, L, num_nodes, num_sub, train_samples, optimizer=optimizers.Adam()):
        """
        ...
        self.eps=eps; self.L=L; self.num_nodes=num_nodes; self.num_sub=num_sub
    ...
    def build_model(self):
        """
        Method for building model according to given parameters in initialization of class 'neural_network'.
        ...
        # define layer structure (-> always dense layer followed by normalization layer)
        layers = []
        ...
        for j in range(self.n):
            strategyhelper = Concatenate()([price,old_strategy])
            ...
            inputs = inputs + [incr]
        #as a result we get input = [w, Z, price, strategy, wealth, incr_1, ..., incr_n] 
        ...
        wealth = Subtract(name='wealth'+str(self.n))([Z,wealth])                # Z-(\delta*S)_T+C_T
        ...
        state = Add()([state,w])
        state=wealth #compute risk according to OCE risk measure
    ...
    def risks(self):
        ...
        samples = model_prices(self.n, self.T, self.num_sub, self.train_samples)
        print('sampling...')
